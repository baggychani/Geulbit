 ...
 * 단일 음절 SVG 렌더러
 */
export default function SyllableRenderer({ char, colors, displaySize = 150 }) {
  ...
  // 색상 변경 시에는 로딩 없이 바로 반영 (경로는 동일)
  // 색상은 렌더링 시 colorMap에서 실시간 반영됨

  if (!char) return null;

  // viewBox: 실제 글리프 영역 기반 (패딩 포함)
  ...
  return (
    <div
      ...
      style={{ width: displaySize, flexShrink: 0 }}
    >
      {/* SVG 글리프 미리보기 */}
      <div
        className="preview-bg-checker group relative flex items-center justify-center"
        style={{
          width: displaySize,
          height: displaySize,
          borderRadius: 12,
      ...
      >
        ...
        {!loading && !error && paths && (
          <svg
            ...
            viewBox={viewBox}
            style={{
              width: displaySize * 0.82,
              height: displaySize * 0.82,
              overflow: 'visible',
            }}
          >
            {paths.map((jp, i) => (
