...
3. **미리보기**에서 결과를 확인합니다. 
   - 크기 S / M / L, 폰트 일반·굵게 조절이 가능합니다.
   - **배치 모드**를 통해 일반 '모아쓰기'와 '자모 그리드' 모드(초·중·종성을 시각적 격자에 맞게 정렬, 이중모음 완벽 지원)를 전환할 수 있습니다.
4. **내보내기** 탭에서 SVG 또는 투명 PNG로 저장합니다. 여러 글자를 ZIP으로 받으면 파일 이름은 입력한 한글을 기준으로 합니다. (예: `안녕하세요_png.zip`)

* 🌍 우측 상단의 언어 설정에서 한국어, 영어, 터키어를 지원합니다.
